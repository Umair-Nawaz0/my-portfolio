# Sardar Umair — AI Systems & Machine Learning Engineering Portfolio

[![Next.js](https://img.shields.io/badge/Next.js-16-black.svg?logo=next.js&logoColor=white)](https://nextjs.org/)
[![React](https://img.shields.io/badge/React-19-61DAFB.svg?logo=react&logoColor=black)](https://react.dev/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0%2B-3178C6.svg?logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![TailwindCSS](https://img.shields.io/badge/TailwindCSS-v4-38B2AC.svg?logo=tailwind-css&logoColor=white)](https://tailwindcss.com/)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

Personal engineering portfolio showcasing production-grade **AI Agents**, **Workflow Automation**, **Machine Learning Systems**, and **C++ Architecture**. Built with modern web standards, server-side rendering, and static site generation via Next.js and TypeScript.

---

## 🚀 Showcased Systems & Repositories

| Project | Category | Key Technologies | GitHub Repository |
| :--- | :--- | :--- | :--- |
| **AI Appointment Assistant** | AI Agents | FastAPI, React 18, TypeScript, PostgreSQL, asyncpg | [appointment-assistant](https://github.com/Umair-Nawaz0/appointment-assistant) |
| **AI Meeting Scheduler** | Automation | n8n, Docker Compose, Zoom OAuth, Google Calendar, Gmail | [meeting-scheduler](https://github.com/Umair-Nawaz0/meeting-scheduler) |
| **Credit Card Fraud Detection** | Machine Learning | Python, Scikit-Learn, SMOTE, LinearSVC | [svm-credit-card-fraud-detection](https://github.com/Umair-Nawaz0/svm-credit-card-fraud-detection) |
| **Loan Default Risk Prediction** | Machine Learning | Python, Scikit-Learn, Decision Trees, GridSearchCV | [decision-tree-loan-default-risk-prediction](https://github.com/Umair-Nawaz0/decision-tree-loan-default-risk-prediction) |
| **Heart Disease Diagnosis** | Machine Learning | Python, Scikit-Learn, Decision Trees, StandardScaler | [decision-tree-heart-disease-prediction](https://github.com/Umair-Nawaz0/decision-tree-heart-disease-prediction) |
| **International Banking Core** | Systems & C++ | Modern C++17, OOP Architecture, File I/O, POSIX | [bank-management-system](https://github.com/Umair-Nawaz0/bank-management-system) |
| **E-Commerce Customer Clustering**| Machine Learning | Python, SciPy, Scikit-Learn, Ward Linkage, PCA | [hierarchical-clustering-customer-segmentation](https://github.com/Umair-Nawaz0/hierarchical-clustering-customer-segmentation) |
| **Retail Customer Segmentation** | Machine Learning | Python, Scikit-Learn, K-Means++, 3D PCA | [kmeans-customer-segmentation](https://github.com/Umair-Nawaz0/kmeans-customer-segmentation) |
| **Salary Prediction Regression** | Machine Learning | Python, Scikit-Learn, Multiple Linear Regression, Plotly | [linear-regression-employee-salary-prediction](https://github.com/Umair-Nawaz0/linear-regression-employee-salary-prediction) |
| **Titanic Survival Classification**| Machine Learning | Python, Scikit-Learn, Logistic Regression, ROC-AUC | [logistic-regression-titanic-survival-prediction](https://github.com/Umair-Nawaz0/logistic-regression-titanic-survival-prediction) |
| **Diabetes Biomarker Prediction** | Machine Learning | Python, Scikit-Learn, K-Nearest Neighbors | [knn-diabetes-prediction](https://github.com/Umair-Nawaz0/knn-diabetes-prediction) |

---

## 🛠️ Tech Stack & Architecture

- **Framework**: [Next.js 16 (Turbopack)](https://nextjs.org/)
- **UI & Components**: [React 19](https://react.dev/), [Base UI](https://base-ui.com/), [Lucide Icons](https://lucide.dev/)
- **Styling**: [Tailwind CSS v4](https://tailwindcss.com/) with custom glassmorphism and animated glow shaders
- **Animation**: [Motion](https://motion.dev/) (Framer Motion)
- **Language**: [TypeScript](https://www.typescriptlang.org/) with strict type validation
- **Routing**: Next.js App Router with SSG (`generateStaticParams`) for instant sub-page navigation

---

## 💻 Local Development Setup

### Prerequisites
- Node.js `20.x` or `22.x`
- npm

### Installation & Execution

```bash
# 1. Clone the repository
git clone https://github.com/Umair-Nawaz0/my-portfolio.git
cd my-portfolio

# 2. Install dependencies
npm install

# 3. Launch development server
npm run dev
```

Open [http://localhost:3000](http://localhost:3000) in your browser.

### Production Build

```bash
npm run build
npm start
```

---

## 👤 Author

**Sardar Umair Nawaz Khan**
- GitHub: [@Umair-Nawaz0](https://github.com/Umair-Nawaz0)
- Email: [sardarumairnawazkhan@gmail.com](mailto:sardarumairnawazkhan@gmail.com)
